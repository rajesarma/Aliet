...
/*function quantityCheck(e,id)	// Pattern like 123456.123.456
{
	...
	// alphas and numbers
		else if ((("0123456789.").indexOf(keychar) > -1))
		{
				...
				if(id.value.split(".").length==2)
				{	
					// don't accept "." up to 3 digits
					if(id.value.split(".")[1].length<3)
					{
						if(keychar==".")
							return false;
						else
					...
					// don't accept numbers if 3 digits
					if(id.value.split(".")[1].length>2)
					{
						if(keychar==".")
							return true;
						else	
				...
				if(id.value.split(".").length==3)
				{	
					// don't accept "." up to 3 digits
					if(id.value.split(".")[2].length<3)
					{
						if(keychar==".")
							return false;
						else
		...
		function regQuantityCheck(text)
		{
		      if(text.value.length>0)
		      {  
		      	if(text.value.match(/^[0-9]*\.[0-9]{3}\.[0-9]{3}/) ==null )
		      	{
		      		text.value="";
		      		return false;	
		      	}
		...
		{
		   if(regQuantityCheck(text))
		   {
		       var text = text.value;
		       
		       var first = text.split(".")[0];
		       var second = text.split(".")[1];
		       var third = text.split(".")[2];
		       
		     //  second = AppendZeros(second);
		     //  third = AppendZeros(third); 
